...
**One caveat though**: `next/root-params` currently doesn't work in Route Handlers or Server Actions. If you're passing an explicit `locale` to functions like `getTranslations` at those call sites, you can incorporate it in `getRequestConfig` as well:
...
2. If your UI renders messages from **multiple locales** in parallel
