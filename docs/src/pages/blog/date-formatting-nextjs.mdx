...
As browsers don't include the time zone of the user in an HTTP request, two ways to retrieve the user's time zone:

1. Pass the user's preferences from the client to the server using a cookie or [the client hints approach](https://github.com/rphlmr/nextjs-client-hints)
2. Use approximate geographical information from the user's IP address, such as the [`x-vercel-ip-timezone`](https://vercel.com/docs/headers/request-headers#x-vercel-ip-timezone) request header if you're running your app on Vercel. However, it's important to note that this is only an approximation, so letting the user choose their time zone explicitly might still be sensible.
