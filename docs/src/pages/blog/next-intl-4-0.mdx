...
So far, when using [`domains`](/docs/routing/configuration#domains) in combination with [`localePrefix: 'as-needed'`](/docs/routing/configuration#locale-prefix-as-needed), `next-intl` had to make some [tradeoffs](https://v3.next-intl.dev/docs/routing/configuration#domains-localeprefix-asneeded) to avoid reading the current host of the incoming request in components.
