...
For quite some time, I used to be a Tailwind skeptic. But one day, I decided to _give it five minutes_ — and I was hooked. Its core idea is just so irresistibly pragmatic. Once you've tried it, you just can't go back.

It made me wonder: "What would the Tailwind for i18n look like?"

Now first of all, `next-intl` is doing very well and has recently crossed [1M weekly downloads](https://www.npmjs.com/package/next-intl) (thank you everyone!). So it seems like there are plenty of people who are quite happy with how it works today. And its core APIs like `useTranslations` are surely here to stay.
...
5. **Minification**: Tailwind class names have a tiny bundle footprint. In the same way, messages should also use minified keys that ensure bundles are as small as possible.
...
Patterns that thwart compile-time rewriting—passing translators down-props/helpers, burying hooks in combinators (`Promise.all`, `.then`), re-export wrappers—trip hard errors now; skim [Static analyzability](/docs/usage/extraction#static-analyzability) when refactoring.
...
Ok, now let's take a step back. Components that use inline messages appear to be easy to generate for AIs, all without having to load lengthy message catalogs into their precious context window.
...
**1. Turbopack loader for extraction**
