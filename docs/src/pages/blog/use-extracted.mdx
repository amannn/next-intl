...
Note that only Next.js 16+ is supported, since it introduced an optimization for Turbopack that allows it to peek inside files to check if they even use the `useExtracted` hook before actually processing the file.
