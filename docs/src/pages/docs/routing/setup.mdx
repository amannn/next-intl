...
Since we are using a dynamic route segment for the `[locale]` param, we need to do one of the following to enable static rendering:

- Pass all possible values to Next.js via [`generateStaticParams`](https://nextjs.org/docs/app/api-reference/functions/generate-static-params) so that the routes can be rendered at build time.
- Return an empty array from `generateStaticParams` to [statically render all paths the first time they're visited](https://nextjs.org/docs/app/api-reference/functions/generate-static-params#all-paths-at-runtime).
- Export `dynamic = 'force-static'` from a layout or page to [force static rendering](hhttps://nextjs.org/docs/app/api-reference/file-conventions/route-segment-config#dynamic).

Depending on your needs, you can add it either to a layout or pages:
...
For statically rendering all locales (this can increase build times):
...
For statically rendering a path when it's first visited you can either return an empty array or use `dynamic = 'force-static'`:
