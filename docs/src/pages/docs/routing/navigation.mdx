...
In case you're using a [`localePrefix`](/docs/routing#locale-prefix) setting other than `always`, you can also enforce a locale prefix with the `forcePrefix` option in case you're changing the user's locale and want to update the [locale cookie](/docs/routing#locale-cookie) first:
