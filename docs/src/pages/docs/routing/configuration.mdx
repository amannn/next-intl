...
The middleware as well as the navigation APIs will automatically consider a [`basePath`](https://nextjs.org/docs/app/api-reference/next-config-js/basePath) that you might have configured in your Next.js config—no additional config is necessary.
...
When using a base path, you should make sure that your middleware [`matcher`](#matcher-config) handles an explicit root:
