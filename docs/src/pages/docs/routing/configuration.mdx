...
Optionally, each domain can override the overall `localePrefix` setting:

```tsx filename="routing.ts" {6,12,18,24}
import {defineRouting} from 'next-intl/routing';
...
Apart from your routing configuration, `next-intl` will also incorporate settings from [`next.config.ts`](https://nextjs.org/docs/pages/api-reference/config/next-config-js). Note that these settings are forwarded by the [`next-intl` plugin](/docs/getting-started/app-router#next-config), therefore make sure it's set up in your app.
