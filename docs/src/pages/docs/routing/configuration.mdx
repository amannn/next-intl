...
Each domain can optionally override the global `localePrefix` mode by specifying a `localePrefix` property. When set, the domain-level setting completely replaces the global mode for that domain:
  ...
  domains: [
    {
      ...
      defaultLocale: 'en-US',
      locales: ['en-US'],
      // Never show locale prefix on this domain
      localePrefix: 'never'
    },
    {
      domain: 'ca.example.com',
      defaultLocale: 'en-CA',
      locales: ['en-CA', 'fr-CA'],
      // Always show locale prefix on this domain
      localePrefix: 'always'
    },
    {
      domain: 'fr.example.com',
      defaultLocale: 'fr-FR',
      locales: ['fr-FR']
      // Uses global localePrefix setting
    }
  ],
  localePrefix: 'as-needed'
});
...
Yes, you can override the global `localePrefix` mode for individual domains using the `localePrefix` property on each domain configuration. When specified, the domain-level setting completely replaces the global mode for that domain, allowing you to have different locale prefix behaviors per domain without needing separate builds.
  ...
  domains: [
    {
      ...
      defaultLocale: 'en-US',
      locales: ['en-US'],
      localePrefix: 'never' // No locale prefix on US domain
    },
    {
      domain: 'ca.example.com',
      defaultLocale: 'en-CA',
      locales: ['en-CA', 'fr-CA'],
      localePrefix: 'always' // Always show locale prefix on CA domain
    }
  ],
  localePrefix: 'as-needed' // Default for domains without localePrefix
});
```

Alternatively, you can build the app for each domain separately while injecting diverging routing configuration via an environment variable.
