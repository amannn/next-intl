...
While you can create entire pages using `page.mdx` files, in an app that uses [the `[locale]` segment](/docs/routing), it can be beneficial to import localized MDX content based on the user's locale into a single `page.tsx` file.
