...
When using [`useExtracted`](/docs/usage/extraction), field-specific messages can be defined with [`msg`](/docs/usage/extraction#msg) at schema-construction time and resolved later in the UI:
...
const passwordRequired = msg('Please enter your password');
...
[Zod](https://zod.dev/) also allows [contextual error maps](https://zod.dev/ERROR_HANDLING?id=contextual-error-map) that customize messages per parse invocation. Since the locale is specific to a particular request, this mechanism comes in handy to turn structured errors from `zod` into localized messages:
