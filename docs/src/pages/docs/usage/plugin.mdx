...
This enables [`useExtracted`](/docs/usage/extraction). Use it together with [`messages`](#messages) and [`srcPath`](#src-path).

- `sourceLocale` — locale new source strings are written to
- `path` — writable catalog directory for extraction (reads existing files here, writes updates). If [`messages.path`](#messages) is a single string, this defaults to that value. **Required** when `messages.path` is an array
- `locales` — target locales kept in sync with `sourceLocale`, or `'infer'` from files in `path` (excluding `sourceLocale`)

`messages.locales` is deprecated but still supported; use `extract.locales`. In `next dev` / `next build`, the plugin warns once if you still use `messages.locales`.
...
When `messages.path` is only your app folder, you can omit `extract.path`:
...
Catalog **loading**: relative path(s) whose files may be imported into the app. The bundler registers a loader for [`format`](#format) under these roots (needed for `.po` and custom formats; `.json` is built-in). This is separate from [`extract.path`](#extract), which is only the **writable** directory used when extracting.
...
Configuring `experimental.messages` sets up the Turbo- or Webpack loader (see [`format`](#format)).

#### `path`

One directory, or several (e.g. app plus a package that ships `.po` files):
...
If you use [`extract`](#extract) and pass an array here, you must set `extract.path` to the catalog directory you want extraction to write to.
