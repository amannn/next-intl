...
Note that this is also relevant when messages are only loaded, since your [`messages.format`](#messages-format) may store source messages differently from translations (e.g. an empty `msgstr` in PO).
...
In either case, the key is stored in `msgctxt` while the original source string is available in `msgid`. Additionally, this format also supports optional descriptions and file references for modules that consume this message.
...
See also the built-in codecs [`@eloqnt/format-po`](https://cli.eloqnt.dev/docs/formats/po) and [`@eloqnt/format-json`](https://cli.eloqnt.dev/docs/formats/json) for inspiration, as well as the supplied types and JSDoc reference.
