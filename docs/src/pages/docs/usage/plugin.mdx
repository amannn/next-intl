...
This is required when [`extract`](#extract) is enabled.
...
Defines the directory where extracted messages should be written to (defaults to [`messages.path`](#messages-path)):
...
It's only required to configure this property when [`messages.path`](#messages-path) is an array.
...
Configuring `experimental.messages` sets up a Turbo- or Webpack loader that allows messages to be imported as plain JavaScript objects (see [`messages.format`](#messages-format)).
...
If you're loading messages from external packages, you can include multiple paths:
...
Defines which locales you translate between in your [`messages.path`](#messages-path).

When using [`useExtracted`](/docs/usage/extraction), this defines which messages are kept in sync when changes to your [`messages.sourceLocale`](#messages-source-locale) are detected (e.g. `en.json`, `de.json`, …).
...
… specify them explicitly (e.g. to use a subset):
...
… or pass an empty array to not extract any messages:
...
In the latter case, you can run extraction manually via [`unstable_extractMessages`](/docs/usage/extraction#manual).

#### `messages.sourceLocale` [#messages-source-locale]

Your primary locale, which might be used to define messages in source code when using [`useExtracted`](/docs/usage/extraction).
...
The PO format is provided by [`@eloqnt/format-po`](https://cli.eloqnt.dev/docs/formats/po) and follows the gettext convention of storing the source text in `msgid`, while the message key is stored in `msgctxt`:
...
This keeps catalogs readable in diffs, PO editors and translation platforms, while [`useExtracted`](/docs/usage/extraction) continues to look up messages by their auto-generated key. Besides the message key and the label itself, this format also supports optional descriptions and file references for modules that consume this message.

Catalogs identify their layout via a header and are written back in the layout they were read in, so files created by earlier versions of next-intl keep working unchanged.
...
##### Migrating PO catalogs [#formats-po-migration]

Earlier versions of next-intl stored the message key in `msgid` and the source text in `msgstr`. Catalogs in that layout are detected via their `X-Crowdin-SourceKey` header and rejected with an error, so a project can't continue with mixed layouts by accident.

To migrate your catalogs, you can use an agent with the following prompt:

> In this project, convert every `.po` messages catalog from the layout where `msgid` holds a message key and `msgstr` holds the text to the gettext-style layout. For each entry in each catalog: set `msgctxt` to the full key (if the file's entries have a `msgctxt` namespace, join it with the `msgid` using a dot), set `msgid` to the entry's source text (look it up by key in the source-locale catalog's `msgstr`; in the source-locale catalog itself it's the entry's own `msgstr`), and keep `msgstr` as it is. In the header block of each file, remove the `"X-Crowdin-SourceKey: msgstr\n"` line and add `"X-Message-Key: msgctxt\n"`. Preserve entry order, comments, references and flags. Verify afterwards by running a build: extraction must not modify the converted files further.

If you'd like to keep the previous layout instead, you can use a [custom format](#formats-custom) based on [`POCodecLegacy`](https://github.com/amannn/next-intl/blob/main/packages/next-intl/src/extractor/format/codecs/fixtures/POCodecLegacy.tsx) (note that it requires `po-parser` as a dependency in your project).
...
Alternatively, a codec can be referenced by the name of an installed package, e.g. `codec: '@eloqnt/format-po'`.

See also the built-in codecs [`@eloqnt/format-po`](https://cli.eloqnt.dev/docs/formats/po) and `@eloqnt/format-json` for inspiration, as well as the supplied types and JSDoc reference.
...
<summary>How can I manually precompile messages?</summary>
