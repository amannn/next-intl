...
**Note:** The `extract` option should be used together with [`messages`](#messages).

#### `extract.sourceLocale` [#extract-source-locale]
...
This defines which messages are kept in sync during extraction (e.g. `en.json`, `de.json`, …).
...
Defines which directory to extract messages to (defaults to `messages.path`).

```tsx
// Extract messages to this directory
path: './messages';
```

#### `extract.srcPath` [#extract-srcpath]

This defines the source path where messages should be extracted to.
...
Configuring `experimental.messages` sets up a Turbo- or Webpack loader that allows messages to be imported as plain JavaScript objects (see [`format`](#messages-format)).
