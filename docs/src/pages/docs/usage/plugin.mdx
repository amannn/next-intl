...
Patterns that break static analyzability (forwarding translators, wrapping imports, unsupported `await getExtracted()` shapes) fail compilation—see [Static analyzability](/docs/usage/extraction#static-analyzability).
