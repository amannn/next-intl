...
This defines where and how message catalogs for locales are stored and loaded.
...
Configuring `experimental.messages` also sets up a Turbo- or Webpack loader so catalogs can be imported as plain JavaScript objects (see [formats](/docs/usage/extraction#formats)).

#### `path`

Relative path to the directory containing message catalogs.

#### `locales`

Defines which locales exist for your catalogs:

- Use `'infer'` to automatically detect locales based on files in `path`
...
When enabled, ICU messages are compiled at build time. This results in **smaller bundles** and **faster message formatting** at runtime.

Notes:

- `t.raw` is not supported with precompiled messages.
- Messages must resolve to strings (no [arrays](/docs/usage/translations#arrays-of-messages) or [objects](/docs/usage/translations#structuring-messages)).

**Note:** `experimental.messages` is required when using [`extract`](#extract). You can also use it without extraction to precompile imported catalogs.
