...
Messages can be extracted as JSON, PO, or custom file formats using codecs.
...
For example, when using `codec: 'po'`, messages can be imported as:
...
Note that JSON files can only hold pairs of keys and values. To provide more context about a message like file references and descriptions, it's therefore recommended to use [PO files](#codecs-po) instead. Alternatively, you can create a [custom codec](#codecs-custom) to store additional metadata.
...
### Custom codec [#codecs-custom]

You can create a custom codec to support any file format. To do this:

1. Create a file that exports a class extending `Codec` from `next-intl/codec`
2. Reference the file path in your configuration

```tsx filename="my-codec.js"
import Codec from 'next-intl/codec';

export default class MyCodec extends Codec {
  // File extension used by this codec
  EXTENSION = '.custom';

  // Decode file content into extracted messages
  decode(content, context) {
    // Parse your custom format and return an array of messages
    // Each message should have at minimum: { id: string, message: string }
    return JSON.parse(content);
  }

  // Encode extracted messages into file content
  encode(messages, context) {
    // Serialize messages to your custom format
    return JSON.stringify(messages, null, 2);
  }

  // Transform file content into JSON string for runtime loading
  toJSONString(content, context) {
    // The returned value will be passed to JSON.parse
    return content;
  }
}
```
...
The `context` parameter passed to codec methods contains:

- `locale`: The locale being processed (e.g., `'en'`, `'de'`)
