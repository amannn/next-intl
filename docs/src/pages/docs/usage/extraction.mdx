...
See also: [Monorepos and external packages](#external-packages-apps)
...
## Monorepos and external packages [#external-packages-apps]
...
Here, you can configure a [`srcPath`](/docs/usage/plugin#src-path) that includes the external package's source directory:

```tsx
const withNextIntl = createNextIntlPlugin({
  experimental: {
    srcPath: ['./src', '../ui/src', './node_modules/@acme/components']
    // ...
  }
});
```
...
If your shared package is used by multiple apps, you might want to reuse all messages among all consumers.
...
In the consuming app, you can configure [`messages.path`](/docs/usage/plugin#messages) to include the external package's messages:

```tsx filename="next.config.ts" {8-9,13-19}
const withNextIntl = createNextIntlPlugin({
  experimental: {
    srcPath: './src',
    extract: {
      ...
      // Extract first-party messages here
      path: './messages'
    },

    messages: {
      // Consider both first-party messages and external ones
      // (e.g. for transformation of .po files)
      path: [
        './messages',
        '../ui/messages',
