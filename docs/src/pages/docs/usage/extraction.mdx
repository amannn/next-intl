...
**App-owned catalogs:** Add other package source roots to [`srcPath`](/docs/usage/plugin#src-path) so all strings extract into the app. When [`messages.path`](/docs/usage/plugin#messages) is a single string, [`extract.path`](/docs/usage/plugin#extract) defaults to it.

**Shared package catalogs:** Run [`unstable_extractMessages`](#manual) in the package (or keep translators’ files there). In the app, use [`messages.path`](/docs/usage/plugin#messages) as an array that lists every directory you import catalogs from (e.g. `./messages` and `../../packages/ui/messages`) so `.po` / custom formats get the loader. Set [`extract.path`](/docs/usage/plugin#extract) to the writable app folder. Merge objects in [`getRequestConfig`](/docs/usage/configuration#messages) if you load more than one file.

E2e coverage for shared-package `.po` imports is planned as a follow-up.
