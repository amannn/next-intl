...
Here, you can configure [`extract.srcPath`](/docs/usage/plugin#extract-srcpath) to include the external package's source directory:
...
In the consuming app, you configure set [`extract.path`](/docs/usage/plugin#extract-path) to only consider your first-party messages, while using [`messages.path`](/docs/usage/plugin#messages-path) to include the external package's messages:
