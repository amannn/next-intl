...
Currently, messages can be extracted as either JSON or PO files. Support for custom codecs is planned for a future release.
...
Note that JSON files can only hold pairs of keys and values. To provide more context about a message like file references and descriptions, it's therefore recommended to use [PO files](#codecs-po) instead. Another alternative will be to use a custom JSON codec in the future.
