...
For details on compiler limitations and unsupported patterns (passing translators, unsupported `await getExtracted()` shapes, imports, and `t.raw`), see [**Static analyzability**](#static-analyzability) below.
...
Message extraction relies on an SWC plugin at compile time. It rewrites `useExtracted` to `useTranslations`, `getExtracted` to `getTranslations`, and replaces each extracted literal with a catalog key only when literals can be tied directly to hook bindings (`const t = useExtracted()` or `const t = await getExtracted()` in supported shapes)—the same invariant future tooling (catalog tree-shaking, AOT compilation, validators) aims to reuse.

Treat the translator as lexical to where you instantiate the hook. Do not pass it onward like a reusable callback (`translate={t}`, `helper(t)`, `{t}`, `[t]`, etc.).

Typical compliant flow:
...
function Example() {
  ...
  function inner() {
    t('scoped');
    t(`also fine`);
    t({message: 'metadata', description: 'tooltip'});
  }
...
Patterns that silently skipped extraction before now fail compilation:

- Forwarding translators (props/arguments/array entries/object literals/returns exposing `t`)
- Non-static message expressions (`t(variable)`, concatenation, templates with `${…}`, dynamically chosen keys). `t.raw` and analogous fully dynamic accessors are incompatible with extraction—combine catalogs plus `useTranslations` when payloads mirror raw blobs ([arrays of messages](/docs/usage/translations#arrays-of-messages) document extractor-friendly structuring for catalog-based workflows)
- Re-export/import indirection wrapping `next-intl` entry points—the plugin only recognizes direct `next-intl` / `next-intl/server` imports
- `await` choreography that hides hooks inside combinators (`Promise.all`, `.then()`, nested expressions)—use `const t = await getExtracted()` (optionally parenthesized) with localized configuration arguments supplied on that hook call alone

Inferring translators across modules or reconstructed parameters demands data flow analysis this extractor intentionally skips—instantiate hooks where JSX runs and localize inline there.
...
The one exception is `t.raw`—pairs poorly with extraction because everything must stay literal-bound (see [**Static analyzability**](#static-analyzability)); structured payloads normally flow through catalogs and `useTranslations` instead ([arrays of messages](/docs/usage/translations#arrays-of-messages)).
...
Messages can be extracted as [`.json`](/docs/usage/plugin#formats-json), [`.po`](/docs/usage/plugin#formats-po), or with [custom file formats](/docs/usage/plugin#formats-custom)—see [`messages.format`](/docs/usage/plugin#format) for configuration details.
...
AI-based translation can be automated with a translation management system like [Crowdin](/docs/workflows/localization-management).
