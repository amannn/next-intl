...
To configure a custom format, you need to specify a codec along with an extension. The codec should implement [`ExtractorCodec`](https://github.com/amannn/next-intl/blob/main/packages/next-intl/src/extractor/format/ExtractorCodec.tsx) from `next-intl/extractor`:

```tsx filename="extractor-codec.ts"
import type {
  ExtractorCodec,
  ExtractorCodecContext,
  ExtractorMessage
} from 'next-intl/extractor';

export default class CustomExtractorCodec implements ExtractorCodec {
  decode(
    content: string,
