...
The same applies to async components using `getExtracted`:

```tsx
import {getExtracted, getTranslations} from 'next-intl/server';

async function AsyncExample() {
  // ✅ Awaiting the hook is supported
  const t = await getExtracted();

  // ✅ Destructuring from `Promise.all` is supported
  ...
  // Therefore, there's no performance need to parallelize
  // it—but it works if you prefer this pattern.
}
```
