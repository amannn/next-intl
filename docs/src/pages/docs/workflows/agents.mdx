...
Learn more in the [introductory blog post](/blog/use-extracted).

## Usage with `useTranslations` [#use-translations]

If you're using [`useTranslations`](/docs/usage/translations), it can be helpful to add instructions to a file like `CLAUDE.md` or `AGENTS.md` to ensure your agent follows your conventions:
...
To address these issues, [`@eloqnt/cli`](https://studio.eloqnt.dev/docs) was created as a companion for `next-intl`. It's a CLI tool that analyzes your source code to enrich messages with usage context, applies per-locale styleguides, and verifies translations after they've been generated.
