...
# Internationalization of Metadata & Route Handlers in Next.js

There are two APIs in Next.js where you might need to apply internationalization outside of React components:

1. [The Metadata API](https://nextjs.org/docs/app/building-your-application/optimizing/metadata)
2. [Route Handlers](https://nextjs.org/docs/app/building-your-application/routing/router-handlers)
...
<VersionTabs items={['Stable', 'Server Components beta version']}>
<Tab>
...
export async function generateMetadata({params: {locale}}) {
  // Passing the `locale` is required. The namespace is optional and
