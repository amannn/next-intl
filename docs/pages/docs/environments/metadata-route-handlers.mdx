...
export async function generateMetadata({params: {locale}}) {
  const t = await getTranslator({locale, namespace: 'Metadata'});
...
export default async function OpenGraphImage({params: {locale}}) {
  const t = await getTranslator({locale, namespace: 'OpenGraphImage'});
...
export async function GET(request) {
  ...
  const t = await getTranslator({locale, namespace: 'Hello'});
