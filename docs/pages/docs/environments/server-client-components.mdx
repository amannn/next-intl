...
Currently, the messages you select for being passed to the client side need to be picked based on knowledge about the implementation of the wrapped components. If this becomes too cumbersome, you can consider [providing all messages to the client side](#option-4-providing-all-messages).
