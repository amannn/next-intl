...
4. **List formatting**: Formatting lists like "HTML, CSS, and JavaScript" is not only a matter of assembling strings in the right order, but also of using the right conjunctions and punctuation marks.
...
1. **Rich text formatting**: Many apps need to embed rich text into text labels (e.g. "Learn more in [the rich text docs](/docs/usage/messages#rich-text).").
...
3. **Caching**: The formatting of messages is cached. Therefore if components are re-rendered with the same data, the previous formatting result can be reused, reducing the amount of necessary computation.
4. **RSC-first**: By integrating deeply with React Server Components, we can offload work to a build step or a capable server, therefore reducing the runtime footprint of your app.
