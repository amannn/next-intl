...
<details>
<summary>How can I pass defaultTranslationValues to NextIntlClientProvider?</summary>

If you want to configure defaultTranslationValues for the NextIntlClientProvider, you will need to render NextIntlProvider as a client component. In order to still receive the messages, call the useMessages hook in a parent component and pass the messages to NextIntlClientProvider as a prop. A common pattern is to use a Providers component to wrap providers that need to render client-side.



<Tabs items={['ParentComponent.tsx', 'Providers.tsx']}>
<Tab>
...
```tsx
const Providers = ({messages, children}) => {
  return <NextIntlClientProvider
    messages={messages}
    defaultTranslationValues={{
      // Add default translation values here
      important: (chunks) => <b>{chunks}</b>,
      value: 123
    }
  >
