...
### Formatting Interpolated daynamic values
...
- `type` is where in the input data to find the data
- `format` is optional, and is a further refinement on how to display that type of data

The available format `type`s available are:

- [`{number}`](/docs/usage/numbers#numbers-within-messages) This type is used to format numbers in a way that is sensitive to the locale.
- [`{date}`](/docs/usage/dates-times#dates-and-times-within-messages) This type is used to format dates in a way that is sensitive to the locale.
- [`{time}`](/docs/usage/dates-times#dates-and-times-within-messages) This type is used to format times in a way that is sensitive to the locale.
- [`{select}`](/docs/usage/plurals-enums#selecting-enum-based-values) This type is used to choose output by matching a value to one of many choices.
- [`{plural}`](/docs/usage/plurals-enums#cardinal-pluralization) This type is used to choose output based on the cardinal pluralization rules of the current locale.
- [`{selectordinal}`](/docs/usage/plurals-enums#ordinal-pluralization) This type is used to choose output based on the ordinal pluralization rules of the current locale.

<Callout type="warning">
  **Important**: If your setup only includes the language in the locale string like this:
...
Formatting might cause hydration errors because the locale strings that you have in your setup only contain the language with no extra data in it to help the [Intl API](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl) match the proper locale rules, so the Intl API resorts to a `localeMatcher` algorithm, and this algorirthm can result in different formatting rules in different hosting environments.
For example, on the server `ar` as a locale string will result in Arabic digits, but on chrome Latin digits.

To solve this hydration error you can override the global formatting options as desired following the Intl API to ensure consistent formatting.
...
export function getGlobalFormatOptions(locale: Locale): IntlConfig['formats'] {
  return {
    dateTime: {
      date: {
        // ...
...
Another solution if you're already handling different locales for the same language, you'll need to add the different locales any way, so you can make your locale strings more specific to make sure that the formatting rules will match on every environment as follows:
...
And adjust the rest of the files accordingly.

Now, this should laser focus your desired locale which can ensure the same formatting rules in different hosting environments.
