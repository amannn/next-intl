...
To render rich text, you'll typically want to use [rich text formatting](#rich-text). However, if you have a use case where you need to emit raw markup, you can use the `t.markup` function:
...
Note that in contrast to `t.rich`, the provided markup functions accept `chunks` as a `string` and have to return a `string` in return.
