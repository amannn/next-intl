...
<Callout>
  Full support for static rendering is currently pending, but [stopgap solutions are available](#static-rendering).
</Callout>
...
Unlike the hooks, these functions require a `locale`. You can use the `locale` that Next.js provides you via [`params`](https://nextjs.org/docs/app/building-your-application/routing/dynamic-routes) and pass them to the function.
...
<Callout>
  The [global request configuration]((#global-request-configuration)) that you've set up in `i18n.ts` is automatically inherited by these functions. The `locale` is the only exception that needs to be provided in comparison to the hooks.
</Callout>
...
Note however that the `useTranslations` hook is the primary API to translate messages and is intended to be used in components. Related: [How (not) to use translations outside of React components](/blog/translations-outside-of-react-components)
