...
Depending on if you handle internationalization in [Client- or Server Components](http://localhost:3000/docs/environments/server-client-components#server-components-benefits), the configuration from `NextIntlClientProvider` or `i18n.ts` will be applied respectively.
