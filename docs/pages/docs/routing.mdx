...
export const routing = defineRouting({
  ...
  defaultLocale: 'en',
  localePrefix:
    process.env.VERCEL_URL === 'us.example.com' ? 'never' : 'always',
  domains: [
    {
      domain: 'us.example.com',
      defaultLocale: 'en',
...
The tradeoff with `redirect` can be mitigated by implementing the redirect either in the middleware or via [`useRouter`](/docs/routing/navigation#usrouter) on the client side. The other ones are however necessary in order to retain the ability to render pages statically.
