...
While the middleware provides a few more options than the navigation APIs, the majority of the configuration is shared between the two and should be used in coordination.
...
By default, the pathnames of your app will be available under a prefix that matches your directory structure (e.g. `src/app/[locale]/about/page.tsx` → `/en/about`). You can however adapt the routing to optionally remove the prefix or customize it per locale.
...
```tsx filename="config.ts"
import {LocalePrefixConfig} from 'next-intl/routing';

export const localePrefix: LocalePrefixConfig = 'always';
```
...
export const localePrefix: LocalePrefixConfig = 'as-needed';
```
...
export const localePrefix: LocalePrefixConfig = 'never';
```
...
export const locales = ['en-US', 'en-GB', 'de-DE', 'zh'] as const;
...
2. You might have to adapt your [middleware matcher](/docs/routing/middleware#matcher-custom-prefix) to match the custom prefixes.
...
<Callout>
  ...
  [`createLocalizedPathnamesNavigation`](/docs/routing/navigation#localized-pathnames)
  instead of `createSharedPathnamesNavigation` for your navigation APIs.
</Callout>
