...
In this example, developers will be prompted to import from `@/routing` when they try to import navigation APIs from Next.js.

```javascript filename="eslint.config.js"
// ...

  rules: {
    // Consistently import navigation APIs from `@/routing`
