...
// Drop-in replacements for Next.js' navigation APIs
// that will consider the routing configuration
export const {Link, redirect, usePathname, useRouter} =
  createNavigation(routing);
```

Depending on your use case, you may wish to customize your routing configuration—more on this later.
...
Note that `NextIntlClientProvider` automatically inherits all configuration from `i18n/request.ts` here—except for the messages.
