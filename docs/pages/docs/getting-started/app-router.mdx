...
<Tabs items={['next.config.mjs', 'next.config.js','next.config.js [without src folder]']}>
<Tab>
...
<Tab>
 ...
 Next.js config without src folder

```js const createNextIntlPlugin = require("next-intl/plugin");
