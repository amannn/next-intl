...
After completing these setup steps, all APIs from `next-intl` can be used in Server Components. If you have a need for using `next-intl` in Client Components too, please refer to [the Server & Client Components guide](/docs/environments/server-client-components).
