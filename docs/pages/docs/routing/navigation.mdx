...
The [navigation APIs](#apis) are strictly typed and only allow routes specified in the `pathnames` config.

If you need to link to unknown routes in certain places, you can either disable the type checking on a case-by-case basis …
