...
By default, the [navigation APIs](#apis) will be strictly typed and only allow routes provided in the `pathnames` config. If you wish to allow unknown routes please use the following type signature (alternatively, you may need to use `@ts-expect-error` for individual cases):
