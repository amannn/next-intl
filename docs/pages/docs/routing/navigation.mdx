...
By default, the [navigation APIs](#apis) will be strictly typed to only allow the routes provided in your `pathnames` config. If you wish to allow unknown routes please use the following type signature (alternatively, you may need to use `@ts-expect-error` for individual cases):

```tsx filename="./navigation.ts"
// ...
export const {Link, redirect, usePathname, useRouter} = createLocalizedPathnamesNavigation({
    locales,
    pathnames: as typeof pathnames & Record<string & {}, string>
});
```
