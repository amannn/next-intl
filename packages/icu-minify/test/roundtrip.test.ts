...
describe('icu-minify', () => {
  ...
  describe('number formatting', () => {
    ...
    it('formats percent', () => {
      ...
      `);
      expect(format(compiled, 'en', {val: 0.75})).toMatchInlineSnapshot(
        `"75%"`
      );
    });
  ...
  describe('select', () => {
    it('selects matching branch', () => {
      ...
      `);
      expect(
        format(compiled, 'en', {gender: 'female'})
    ...
    it('throws for select without other', () => {
      expect(() =>
        compile('{gender, select, female {She} male {He}}')
  ...
  describe('nesting', () => {
    ...
    it('formats deeply nested structures', () => {
      ...
      expect(format(compiled, 'en', {a: 'x', b: 1, c: 'y'})).toMatchInlineSnapshot(
        `"deep"`
      );
      expect(format(compiled, 'en', {a: 'x', b: 1, c: 'z'})).toMatchInlineSnapshot(
        `"nested"`
      );
      expect(format(compiled, 'en', {a: 'x', b: 2, c: 'y'})).toMatchInlineSnapshot(
        `"level"`
      );
      expect(format(compiled, 'en', {a: 'z', b: 1, c: 'y'})).toMatchInlineSnapshot(
        `"top"`
      );
    });
  });
  ...
  describe('locales', () => {
    it('uses Polish plural rules', () => {
      ...
      expect(format(compiled, 'pl', {n: 5})).toMatchInlineSnapshot(
        `"5 plików"`
      );
      expect(format(compiled, 'pl', {n: 22})).toMatchInlineSnapshot(
        `"22 pliki"`
      );
    });
    ...
    it('uses Russian plural rules', () => {
      ...
      expect(format(compiled, 'ru', {n: 5})).toMatchInlineSnapshot(
        `"5 файлов"`
      );
      expect(format(compiled, 'ru', {n: 21})).toMatchInlineSnapshot(
        `"21 файл"`
      );
    });
