...
[Support for Server Components in `next-intl`](/docs/next-13/server-components) will allow you to use the same APIs as in other components. For the meantime, as a stopgap solution, you can use the [core library](/docs/usage/core-library) from `next-intl`.
