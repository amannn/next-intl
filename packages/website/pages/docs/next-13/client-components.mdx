...
2. Pass through children in `app/layout.tsx`:
...
3. Provide the document layout and set up `NextIntlClientProvider` in `app/[locale]/layout.tsx`:
...
4. Turn your component in `app/[locale]/page.tsx` into a Client Component to be able to use translations:
