...
The simplest option is to create JSON files locally that are separated by a [locale](<https://en.wikipedia.org/wiki/Locale_(computer_software)>).
...
The `locale` that was matched by the middleware is available via `useLocale` everywhere in your app. As a first step, use it to configure the document language.
...
```tsx filename="app/[locale]/Expandable.tsx" {5,14-15}
'use client';
