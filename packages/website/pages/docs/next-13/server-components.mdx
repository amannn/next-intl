...
```
npm install next-intl@2.11.0-beta.6
```
...
2. `next-intl` creates an i18n configuration (short for "internationalization") once per request and makes it available to all Server Components. Define this in `i18n.tsx`:
...
export default getRequestConfig(async ({locale}) => ({
  ...
  timeZone: headers().get('x-time-zone') ?? undefined
}));
```
