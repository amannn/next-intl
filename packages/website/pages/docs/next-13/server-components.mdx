...
- 🚧 Bugs were discovered that probably need to be resolved upstream in React/Next.js (workarounds are available).
...
5. Provide the document layout and set up `NextIntlServerProvider` in `app/[locale]/layout.tsx`:
