...
5. Pass through children in `app/layout.tsx`:
...
6. Provide the document layout and set up `NextIntlServerProvider` in `app/[locale]/layout.tsx`:
...
7. Use translations in your page component in `app/[locale]/page.tsx` or anywhere else!
...
// When the user is on `/en`, the link will point ot `/en/about`
