...
### Example: Integrating with [Auth.js](https://authjs.dev/) (aka. NextAuth.js)

Combining the middlewares of Auth.js and `next-intl` is a bit tricky to get right.

Auth.js provides a [success callback](https://next-auth.js.org/configuration/nextjs#wrap-middleware) that we can use to run the `next-intl` middleware on authorized pages. However, public pages need to be treated separately.
...
There's a working [example that combines `next-intl` with Auth.js](https://github.com/amannn/next-intl/tree/docs/middleware-composition-examples/packages/example-next-13-next-auth) on GitHub.
