...
Yes. These environments pose the challenge that components render in isolation and therefore relevant providers might be missing. To fix this, you should wrap the component with the `NextIntlProvider`.

Please see the [Jest example](https://github.com/amannn/next-intl/blob/main/packages/example-advanced/src/components/Navigation.spec.tsx) to explore a working setup. For Storybook, you have to [add a decorator](https://storybook.js.org/docs/react/writing-stories/decorators) that configures the provider.
