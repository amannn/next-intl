...
export default function Polyfills() {
  const topLevelLocale = useLocale().replace(/-.*/, '');

  const polyfills = [
    'Intl',
    'Intl.DateTimeFormat',
    `Intl.DateTimeFormat.~locale.${topLevelLocale}`,
    `Intl.NumberFormat.~locale.${topLevelLocale}`,
    'Intl.PluralRules',
    `Intl.PluralRules.~locale.${topLevelLocale}`,
    'Intl.RelativeTimeFormat',
    `Intl.RelativeTimeFormat.~locale.${topLevelLocale}`
  ];
