...
If you work with translators, it can be helpful for them to use an editor that supports the ICU syntax for dates and times (e.g. the <PartnerContentLink href="https://support.crowdin.com/icu-message-syntax/#date-time">Crowdin Editor</PartnerContentLink> handles the default formats of `next-intl`).
