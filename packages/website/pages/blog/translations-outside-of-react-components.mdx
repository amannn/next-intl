...
Have you ever wondered why `next-intl` doesn’t provide an API to consume i18n translations outside of React components?
...
This may seem like an unnecessary limitation, but the absence of this feature is in fact intentional and aims to encourage the use of proven patterns that avoid potential issues—especially if they are easy to overlook.
...
Now, we can offer a better user experience by interactively counting down the time to the next attempt. Additionally, this approach is more robust to possibly unexpected states, like the user changing the language while the timeout message is being displayed.
...
By limiting ourselves to only format messages during render, we experience a similar situation: The rendered output of translated messages is always in sync with app state.

## The exception that proves the rule

If you’re working with Next.js, you might need to translate i18n messages in [API routes](https://nextjs.org/docs/api-routes/introduction), or in case you're using the App Router, in [Route Handlers](https://beta.nextjs.org/docs/routing/route-handlers) or for the [Metadata API](https://beta.nextjs.org/docs/api-reference/metadata).

`next-intl` provides a [core library](/docs/usage/core-library) that is agnostic from React and can be used for these use cases.
...
Furthermore, the `next-intl` Server Components beta [proposes APIs to further simplify this use case](/docs/next-13/server-components#using-internationalization-outside-of-components).
