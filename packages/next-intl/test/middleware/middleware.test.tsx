...
describe.each([{basePath: ''}, {basePath: '/sample'}])(
  'prefix-based routing with basePath: $basePath',
    ...
    describe('localePrefix: as-needed', () => {
      ...
      describe('localized pathnames', () => {
        ...
        it('redirects when a pathname from the default locale ends up with a different locale', () => {
          ...
          middlewareWithPathnames(
            createMockRequest('/de/users/2?page=1', 'de')
          );
          expect(MockedNextResponse.rewrite).not.toHaveBeenCalled();
          expect(MockedNextResponse.next).not.toHaveBeenCalled();
      ...
      describe('localized pathnames with different pathnames for internal and external pathnames for the default locale', () => {
        ...
        it('redirects a request for a localized route to remove the locale prefix while keeping search params', () => {
          middlewareWithPathnames(
            createMockRequest('/en/external?hello', 'en')
          );
          expect(MockedNextResponse.next).not.toHaveBeenCalled();
          expect(MockedNextResponse.rewrite).not.toHaveBeenCalled();
          expect(MockedNextResponse.redirect).toHaveBeenCalledTimes(1);
    ...
    describe('localePrefix: always', () => {
      ...
      describe('localized pathnames', () => {
        ...
        it('serves requests for a non-default locale at nested paths', () => {
          ...
          middlewareWithPathnames(
            createMockRequest(
              '/de/neuigkeiten/gutes-neues-jahr-g5b116754',
              'de'
            )
          );
...
describe.each([{basePath: undefined}, {basePath: '/sample'}])(
  'domain-based routing with basePath: $basePath',
